...
import { Calendar, Clock, CheckCircle, XCircle, User, AlertCircle, Shield } from "lucide-react"
...
export default function ManagerDashboard() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await fetch("/api/requests/pending/manager")
      if (response.ok) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 tracking-tight">Manager Dashboard</h1>
            <p className="mt-2 text-lg text-gray-600">Review supervisor-approved absence requests</p>
          </div>
          <LogoutButton />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className="border-gray-200 shadow-xl">
              <CardHeader>
                <CardTitle>Supervisor-Approved Requests</CardTitle>
                <CardDescription>Second-level approval for absence requests</CardDescription>
              </CardHeader>
              <CardContent>
                {requests.length === 0 ? (
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            {request.supervisor_name && (
                              <p className="text-xs text-gray-500 mt-1">Approved by: {request.supervisor_name}</p>
                            )}
                          </div>
                ...
                {selectedRequest ? (
                  <div className="space-y-4">
                    <div className="p-4 bg-purple-50 border border-purple-200 rounded-lg">
                      <p className="text-sm font-semibold text-purple-900 mb-2">Request Details</p>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-purple-700">Employee:</span>
                          <span className="font-medium text-purple-900">{selectedRequest.employee_name}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-purple-900">{selectedRequest.total_days} days</span>
                        </div>
                      </div>
                    </div>
